...
def load_pkl(relative_path):
    ...
    # Auto-fixing path to include Models_New and the CVD_RIsk folder typo
    fixed_path = relative_path.replace("CVD_Risk", "CVD_RIsk")
    full = os.path.join(BASE, "Models_New", fixed_path)
    ...
    try:
        with open(full, "rb") as f:
            obj = pickle.load(f)
        print(f"  [OK] Loaded: {relative_path}  [{type(obj).__name__}]")
        return obj
    except Exception as e:
        print(f"  [ERROR] Error loading {relative_path}: {e}")
        return None
...
FEAT = {
    "cvd":         resolve_features(cvd_model, None, CVD_FEATURES),
    "diabetes_A":  resolve_features(diabetes_model_A, diabetes_features_A, DIABETES_FEATURES_A_DEFAULT),
    "diabetes_B":  resolve_features(diabetes_model_B, diabetes_features_B, DIABETES_FEATURES_B_DEFAULT),
...
def build_X(patient_data: dict, feature_list: list, medians: dict = {}) -> np.ndarray:
...
def get_shap_values(model_key, model, explainer_pkl, X, feature_list):
    ...
    try:
        ...
        total = float(np.abs(sv).sum() or 1)
        result = []
...
def predict_all(patient: Dict[str, Any]):
    results = {}
    probs   = {}
...
def predict_single(model_key: str, patient: Dict[str, Any]):
    model_lookup = {
        "cvd":         (cvd_model,        "cvd",         {},
                        "CVD / Heart Disease Risk", 0.9914),
        "diabetes_A":  (diabetes_model_A, "diabetes_A",  MEDIANS["diabetes"],
...
def explain(model_key: str, patient: Dict[str, Any]):
    explainer_lookup = {
...
def simulate(data: Dict[str, Any]):
    ...
    modified.update({k: float(v) for k, v in intervention.items()})
